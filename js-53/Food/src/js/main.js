...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function formData(form){
        form.addEventListener('submit', (event) => {
            ...
            elem.classList.add('status-loading');
            elem.textContent = messages.loading;
            form.append(elem);
            ...
            /* когда использую XMLHttpRequest + FormData, то заголовок устанавливать не надо!
            request.setRequestHeader('Content-type', 'multipart/form-data');*/

            // formData - для передачи данных формы без перебора каждого элемента формы
            ...
            request.addEventListener('load', () => {
                if(request.status == 200){
                    elem.textContent = messages.access;
                    form.reset();
                    ...
                    elem.textContent = messages.fail;
